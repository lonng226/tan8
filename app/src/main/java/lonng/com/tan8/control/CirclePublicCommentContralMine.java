...
*
 */
public class CirclePublicCommentContralMine {
	private static final String TAG = CirclePublicCommentContralMine.class.getSimpleName();
	...
	public CirclePublicCommentContralMine(MineFragment context, View editTextBody, EditText editText, View sendBt){
		mContext = context;
...
//					mCirclePresenter.addComment(mCirclePosition, mCommentType, mReplyUser);
	...
	* @throws
	...
	public void editTextBodyVisible(int visibility, CirclePresenter mCirclePresenter, int mCirclePosition,
									int commentType, User replyUser, int commentPosition,int Tid) {
	...
	public void handleListViewScroll() {
		...
		int keyH = TanApplication.mKeyBoardH2;//键盘的高度
		int editTextBodyH = (mContext).getEditTextBodyHeight();//整个EditTextBody的高度
		int screenlH = (mContext).getScreenHeight();//整个应用屏幕的高度
...
//		Log.d(TAG, "offset=" + listviewOffset + " &mSelectCircleItemH=" + mSelectCircleItemH + " &keyH=" + keyH + " &editTextBodyH=" + editTextBodyH);
		...
		if(mListView!=null){
			...
			mListView.setSelectionFromTop(mCirclePosition, listviewOffset - (mContext).getTitleLayoutHeight());
		}
