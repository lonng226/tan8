...
public class CirclePublicCommentContral {
	...
	public CirclePublicCommentContral(BasePage context, View editTextBody, EditText editText, View sendBt){
		mContext = context;
...
//					mCirclePresenter.addComment(mCirclePosition, mCommentType, mReplyUser);
	...
	* @throws
	...
	public void handleListViewScroll() {
		...
		int editTextBodyH = ((Czxpage)mContext).getEditTextBodyHeight();//整个EditTextBody的高度
		...
		if(mListView!=null){
			...
			mListView.setSelectionFromTop(mCirclePosition, listviewOffset - ((Czxpage)mContext).getTitleLayoutHeight());
		}
